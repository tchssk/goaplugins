...
func init() {
	codegen.RegisterPlugin("codegendisabler-gen-http-server-server-servermountpoint", "gen", nil, Generate)
}
...
func Generate(genpkg string, roots []eval.Root, files []*codegen.File) ([]*codegen.File, error) {
	for _, f := range files {
		...
		for _, section := range f.SectionTemplates {
			if section.Name != "server-mountpoint" {
